...
export default function PerformanceReviewsPage() {
    ...
    const canAccess = isAuthenticated; // All authenticated users can access history
    ...
    const loadPerformances = async () => {
        try {
            ...
            // RBAC filtering: Managers only see EMPLOYEE reviews
            if (currentUser?.role === Role.MANAGER) {
                setPerformances(data.filter((p) => {
                    const targetRole = p.employee?.userId?.role;
                    return targetRole === 'EMPLOYEE' || targetRole === 'CLERK';
                }));
            } else if (currentUser?.role === Role.ADMIN) {
                // Admin sees everything
                setPerformances(data);
            } else {
                // Regular employees see ALL their own reviews (including pending)
                setPerformances(data);
            }
        } catch (error) {
    ...
    const canEdit = (performance: Performance) => {
        if (!currentUser) return false;

        // Admin can edit anyone's review
        if (currentUser.role === Role.ADMIN) return true;

        // Manager can only edit employee reviews
        if (currentUser.role === Role.MANAGER) {
            const targetRole = performance.employee?.userId?.role;
    ...
    const handleEditSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await performanceService.updatePerformance(editingPerformance.id, editFormData);
            await loadPerformances();
            setEditingPerformance(null);
        } catch (error: any) {
            alert(error.response?.data?.message || 'Failed to update performance review');
        } finally {
                        ...
                        <p className="text-slate-600 dark:text-slate-400 mt-1">
                            {currentUser?.role === Role.MANAGER
                                ...
                                : currentUser?.role === Role.ADMIN
                                    ...
                                    : 'View your performance review history and self-assessments'}
                        </p>
                    </div>
                    <button
                        onClick={() => router.push('/performance/add')}
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div>
                                            ...
                                            <p className="text-2xl font-bold text-green-600 dark:text-green-400 mt-1">
                                                {performances.filter((p) => (p.rating || 0) >= 4.5).length}
                                            </p>
                                        </div>
                            ...
                            <div className="p-6 space-y-4">
                                {loading ? (
                                    ...
                                    performances.map((performance) => (
                                        <div
                                            key={(performance as any)._id || performance.id}
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <div className="flex items-center gap-2 mb-4">
                                                        {(performance.rating || 0) > 0 ? (
                                                            <>
                                                                <div className="flex">{getRatingStars(performance.rating || 0)}</div>
                                                                <span className={`text-lg font-bold ${getRatingColor(performance.rating || 0)}`}>
                                                                    {(performance.rating || 0).toFixed(1)}
                                                                </span>
                                                            </>
                                                        ) : (
                        ...
                        {editingPerformance && (
                            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                                <motion.div
                                    ...
                                    className="bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 w-full max-w-lg overflow-hidden"
                                >
                                        ...
                                        <table className="w-full text-left">
                                            ...
                                            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                                {analytics.topPerformers.map(p => (
                                                    <tr key={(p.employee as any)._id || p.employee.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
